...
Listado.prototype.buscarRestaurante = function(id) {
   var busqueda = this.restaurantes.find(resto => resto.id === id);
    if (busqueda == null){busqueda = "No se ha encontrado ningún restaurant"}
    return busqueda;
}
...
Listado.prototype.obtenerCiudades = function() {
    ...
    //funcion para agregar las ciudades al arreglo
    ciudades = this.restaurantes.map(function(restaurante){return restaurante.ubicacion})
...
Listado.prototype.obtenerRubros = function() {
    ...
    rubros = this.restaurantes.map(function(restaurante){return restaurante.rubro});
...
Listado.prototype.obtenerHorarios = function() {
    ...
    var arregloTodosHorarios = [];

    //Recorremos el array de restaurantes y vamos agregando todos los array de horarios
    arregloTodosHorarios = this.restaurantes.map(function(restaurante){return restaurante.horarios});

    //En este arreglo vamos a poner todos los horarios, uno por uno
    var horarioRes = [];
    arregloTodosHorarios.forEach(function(horario) {
        horario.forEach(function(hora) {
            horarioRes.push(hora)
        });
    });

 
    // En este arreglo vamos a poner todos los horarios pero sin repetidos
    var horariosUnicos = eliminarRepetidos(horarioRes)

    return horariosUnicos.sort();
}

//funcion que devuelve el arreglo pasado por parametro sin elementos repetidos
function eliminarRepetidos(arreglo){
    var unicos = arreglo.filter(function(elem, index, self) {
